    ...
    for page in range(totalPages):
        ...
        try:
            ...
            for li in soup.select('.item.fl'):
                ...
                searchPrice = li.select('.i-collect.searchBg.collectI')
                strPrice = ''
                if searchPrice:
                    retMatch = re.findall(r'(\d+),.*?(\d+)',searchPrice[0]['href'])
                    if retMatch:
                        (le,ri) = retMatch[0]
                        if le and ri:
                            strPrice = ri + '_' + le
...
def parseCommandLine():
    ...
    for opt, value in options:
        if opt in ['--level1','--level2','--level3']:
            strKey = re.sub('-','',opt)
            para[strKey] = value.decode('utf-8').split(',')
        elif opt in ['--site','--database','--catTable','--productTable']:
            strKey = re.sub('-','',opt)
            para[strKey] = value.decode('utf-8')
