...
def parseCommandLine():
    para = {}
    options,args = getopt.getopt(sys.argv[1:],"h",['site=', 'level1=', 'level2=', 'level3=', 'host=', 'port=', 'database=','pagesize=', 'hasPrice','batchUpdate', 'hasSpec','delta', 'help','catUpdate'])
    for opt, value in options:
        if opt in ['--level1','--level2','--level3']:
            strKey = re.sub('-','',opt)
            para[strKey] = value.decode('utf8').split(',')
        elif opt in ['--site']:
            strKey = re.sub('-','',opt)
            para[strKey] = value.decode('utf8')
        elif opt in ['--host','--port','--database','--pagesize']:
            strKey = re.sub('-','',opt)
            para[strKey] = value
...
def usage():
    print "Usage: python getCategory.py [--help] [--site] [--hasPrice] [--hasSpec] [--homeUrl]  [--host], [--port], [--database], [--level1] [--level2] [--level3] [--delta] [--batchUpdate] [--catUpdate]\n"
